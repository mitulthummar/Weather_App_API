...
const getWeatherData = async (req, res) => {
    try {
        ...
        const unitType = units === 'Celsius' ? 'metric' : 'imperial';
        ...
        https.get(url, (response) => {
            ...
            response.on('data', (data) => {
                ...
                let sunriseFormatted = `${sunriseTime.getHours()}:${sunriseTime.getMinutes()}:AM`;
                let sunsetTimestamp = weatherData.sys.sunset * 1000; // Convert to milliseconds
                let sunsetTime = new Date(sunsetTimestamp);
                let sunsetFormatted = `${sunsetTime.getHours()}:${sunsetTime.getMinutes()}:PM`;
                ...
                let amOrPm = curDate.getHours() < 12 ? 'AM' : 'PM';
                let formattedDate = `${day}/${month}/${year} ${hours}:${minutes}:${seconds} ${amOrPm}`;
                ...
                const temperatureType = units === 'Celsius' ? 'degree Celsius' : 'Fahrenheit';
